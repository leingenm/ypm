...
public class YouTubePlaylistService implements PlaylistService {
    ...
    public Playlist createPlaylist(Playlist playlist) {
        ...
        ytPlaylist
            .setStatus(new PlaylistStatus().setPrivacyStatus(
                requireNonNull(playlist.getPrivacyStatus()).getValue()))
            .setSnippet(new PlaylistSnippet().setTitle(playlist.getTitle())
    ...
    public Playlist updatePlaylistTitle(String playListId, String newTitle) {
        ...
        try {
            var updatedYtPlaylist = youTubeClient
                .playlists()
                .update(List.of(Part.SNIPPET.toString(), Part.STATUS.toString()),
                    ytPlaylistForRequest)
                .setAccessToken(authService.getToken())
                .execute();
    ...
    public Playlist mergePlaylists(
        ...
        for (var playListId : playListsIds) {
            ...
            moveVideosBetweenPlaylists(playListId, resultingPlaylist.getId(), videosIds,
                deleteAfterMerge);
            if (deleteAfterMerge) {
    ...
    public Playlist moveVideosBetweenPlaylists(
        ...
        for (Video video : videosToMove) {
            if (requireNonNull(video.getPlaylistId()).equals(playListId)) {
                ...
                if (deleteAfterMove) {
                    removeVideoFromPlaylist(video.getPlaylistItemId());
                }
            }
        }
